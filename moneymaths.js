...
let dailyPot = 1000000; // ₦1,000,000 for simulation
let currentProblems = [];
const NUM_BLOCKS = 8;
let trainActive = false;
...
function generateProblems() {
  ...
  // Show verify button only during play
...
joinTrainBtn.addEventListener('click', () => {
  if (dailyPot <= 0) {
    showPopup("🚫 Train Station Closed for Today! Check back tomorrow.");
    joinTrainBtn.disabled = true;
    return;
  }
...
verifyBtn.addEventListener('click', () => {
  ...
  if (!correct) {
    showPopup("⚠️ You’re not yet done hashing your train ticket, hurry!");
    return;
  }

  const reward = Math.floor(Math.random() * 5000) + 1000; // ₦1k–₦6k
  const destination = document.getElementById('trainDestination').textContent;
  playSound(whistle);

  setTimeout(() => {
    showPopup(`🎫 You’ve secured your ${destination} train ticket number ${ticketNumber}! You’ve earned ₦${reward}!`);
  }, 400);

  dailyPot -= reward;
  dailyPotEl.textContent = dailyPot.toLocaleString();

  // End game and check if pot empty
  problemBoard.style.display = 'none';
  verifyBtn.style.display = 'none';
  joinTrainBtn.style.display = 'block';
  joinTrainBtn.textContent = 'Join Next Train';
  joinTrainBtn.style.display = 'block';
