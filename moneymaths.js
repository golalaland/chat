...
const dailyPotEl = document.getElementById('dailyPot');
let dailyPot = 1000000;
const increment = 10;
const maxPot = 10000000;
...
function updateOdometer(num) {
  const digits = getDigits(num);
  const digitSpans = dailyPotEl.querySelectorAll('.digit span');
  
  digits.forEach((d, i) => {
    const span = digitSpans[i];
    const currentDigit = parseInt(span.textContent, 10) || 0;
    if (currentDigit !== parseInt(d, 10)) {
      // move vertically
      span.style.transform = `translateY(-${parseInt(d, 10) * 100}%)`;
      span.textContent = d;
    }
  });
}
...
function tickPot() {
  if (dailyPot < maxPot) {
    dailyPot = Math.min(dailyPot + increment, maxPot);
    updateOdometer(dailyPot);
  } else {
    setTimeout(() => {
      dailyPot = 1000000;
      updateOdometer(dailyPot);
...
// Initialize: add inner spans if missing
dailyPotEl.querySelectorAll('.digit').forEach(d => {
  if (!d.querySelector('span')) {
...
updateOdometer(dailyPot);
setInterval(tickPot, intervalMs);