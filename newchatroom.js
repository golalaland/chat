...
function setupUsersListener(){
  onSnapshot(collection(db,"users"),snap=>{
    ...
    snap.forEach(d=>{ refs.userColors[d.id]=d.data()?.usernameColor||"#fff"; });
    if(lastMessagesArray.length) renderMessagesFromArray(lastMessagesArray);
  });
}
setupUsersListener();

/* ---------- Message rendering with Twitch-style scroll ---------- */
...
async function loginWhitelist(email,phone){
  try{
    ...
    if(!docSnap.exists()){
      ...
      showStarPopup("🎉 Account created with 50 stars!");
    ...
    // Hide login/sign-in elements
    if(refs.authBox) refs.authBox.style.display = "none";
    const roomSubtitle = document.getElementById("roomSubtitle");
    if(roomSubtitle) roomSubtitle.style.display = "none";
...
function startStarEarning(uid){
  ...
  starInterval = setInterval(async()=>{
    ...
    if(data.lastStarDate!==today) await updateDoc(userRef,{starsToday:0,lastStarDate:today});
...
window.addEventListener("DOMContentLoaded",()=>{
  ...
  refs.sendBtn?.addEventListener("click",async()=>{
    ...
    if(!isFree){ currentUser.stars-=SEND_COST; refs.starCountEl.textContent=formatNumberWithCommas(currentUser.stars);
  ...
  refs.buzzBtn?.addEventListener("click",async()=>{
    ...
    const docRef = await addDoc(collection(db,CHAT_COLLECTION),{
      content:txt, uid:currentUser.uid, chatId:currentUser.chatId,
      timestamp:serverTimestamp(), highlight:true, buzzColor:color
    });
    refs.messageInputEl.value="";
    showStarPopup("BUZZ sent!");
    renderMessagesFromArray([{id:docRef.id,data:{content:txt,uid:currentUser.uid,chatId:currentUser.chatId,highlight:true,buzzColor:color}}],true);
  });
